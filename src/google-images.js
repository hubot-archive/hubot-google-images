...
module.exports = (robot) => {
  ...
  const imageMe = (msg, query, animated, faces, cb) => {
    ...
    if (googleCseId) {
      ...
      if (!googleApiKey) {
        robot.logger.error('Missing environment variable HUBOT_GOOGLE_CSE_KEY');
      ...
      msg.http(url)
        ...
        .get()((err, res, body) => {
          ...
          if (response.error != null ? response.error.errors : undefined) {
            response.error.errors.forEach((error) => {
              robot.logger.error(error.message);
              if (error.extendedHelp) {
                robot.logger.error(`(see ${error.extendedHelp})`);
              }
            });
          }
        });
  ...
  robot.respond(/(?:mo?u)?sta(?:s|c)h(?:e|ify)?(?: me)? (.+)/i, (msg) => {
    ...
    const mustacheBaseUrl = process.env.HUBOT_MUSTACHIFY_URL != null ? process.env.HUBOT_MUSTACHIFY_URL.replace(/\/$/, '') : undefined;
